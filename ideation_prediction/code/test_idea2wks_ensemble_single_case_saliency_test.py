...
if args.new_subsample:
    ...
    df_test = convert_total_quantile(args, df_test, dataset=None)
...
sequential_sampler = BatchSampler(SequentialSampler(range(lst_test_len)), batch_size=1, drop_last=False)#(dataset['test'])
...
indiv_figpath = os.path.join(args.result_dir, 'single', 'individual maps')
if not os.path.exists(indiv_figpath):
    # shutil.rmtree(indiv_figpath)
    os.makedirs(indiv_figpath)
    
avg_figpath = os.path.join(args.result_dir, 'single', f'average maps')
...
for batch, (bg, label) in enumerate(dataloader['test']):
    ...
    for test_num in np.arange(1, num_graphs+1):
        ...
        # print(f'batch/test_num:{batch}/{test_num}')
        ...
        if draw_indiv_saliency:
            ...
            print(f'test_idx:{test_idx+1}/{lst_test_len} th data')
...
# load pred and true label
ensemble_result_dir = os.path.join(args.result_dir, 'single')

with open(os.path.join(ensemble_result_dir, 'single_pred_true_score.pkl'), 'rb') as f:
      true_pred_score = pickle.load(f)

args.true_pred_score = true_pred_score 

plot_saliency_maps_for_single(args, avg_figpath, saliency_test_dict)
